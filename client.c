					// It wil include  the client  side code 
					//argv[0] filename
					//argv[1] server_ipaddr
					//argv[2] port_no
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/types.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <netdb.h> //hostent struct
#include <unistd.h> //read,write,close

void error(const char *msg){perror(msg);exit(1);} //for system errors only, not for user side errors, bindor accept or socket(){these are system functions} generally fails and hence we use this. 

int main(int argc , char *argv[]){
	int sockfd, portno, n;
	struct sockaddr_in server_addr;
	struct hostent *server;

	char buffer[255];
	if(argc<3){fprintf(stderr,"usage %s hostname port",argv[0]); exit(1);}

	portno = atoi(argv[2]);

	sockfd = socket(AF_INET, SOCK_STREAM, 0);
	if(sockfd<0){error("failed to create a socket \n");}

	server = gethostbyname(argv[1]);
	if(server == NULL){fprintf(stderr, "no such hosts \n");}

	bzero((char *)&server_addr, sizeof(server_addr));	
	server_addr.sin_family = AF_INET;
	memcpy(&server_addr.sin_addr.s_addr,
       server->h_addr_list[0],
       server->h_length);
	server_addr.sin_port = htons(portno);

	if(connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr))<0){error("failed to connect \n");}

	while(1){

		bzero(buffer, 255);
		fgets(buffer, 255, stdin);
		n = write(sockfd, buffer, strlen(buffer));
		if(n<0){error("failed to write \n");}

		bzero(buffer, 255);
		n = read(sockfd, buffer, 255);
		if(n<0){error("failed to read \n");}

		printf("SERVER : %s", buffer);

		int i = strncmp("bye", buffer, 3);
		if(i == 0){break;}
	}

	close(sockfd);
	return 0;
}